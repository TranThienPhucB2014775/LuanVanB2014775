		...
		<ScrollArea className="h-[calc(100vh-4rem)] px-4 py-6">
			<div className="container max-w-3xl mx-auto">
				<Card className="w-full">
					<CardHeader>
						<CardTitle className="text-2xl font-bold text-center">Cập nhật thông tin cho thuê</CardTitle>
					</CardHeader>
					<CardContent>
						<Form {...form}>
							<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
								<div className="space-y-4">
																...
																<CommandList>
																	...
																	<CommandGroup>
																		{vietnamCities.map((cityData) => (
																			...
																			>
																				<Check
																					className={cn("mr-2 h-4 w-4", city === cityData.city ? "opacity-100" : "opacity-0")} />
																				{cityData.city}
																			</CommandItem>
																		))}
																	</CommandGroup>
													...
													<Popover>
														...
														<PopoverContent className="w-full p-0">
															<Command>
																...
																<CommandList>
																	<CommandEmpty>Không tìm thấy
																		quận/huyện.</CommandEmpty>
																	<CommandGroup>
																		{city && vietnamCities.find(item => item.city === city)?.districts.map((district) => (
																			...
																			>
																				<Check
																					className={cn("mr-2 h-4 w-4", form.watch("district") === district ? "opacity-100" : "opacity-0")} />
																				{district}
																			</CommandItem>
																		))}
																	</CommandGroup>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Textarea
														placeholder="Nhập mô tả chi tiết về bất động sản" {...field}
														className="h-32" />
												</FormControl>
												<FormMessage />
											</FormItem>
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input placeholder="Ví dụ: Wi-Fi, máy lạnh, tủ lạnh" {...field} />
												</FormControl>
												<FormMessage />
											</FormItem>
										)}
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
												...
												<FormItem>
													...
													<FormControl>
														<Input
															...
															{...field}
															onChange={(e) => form.setValue("area", parseInt(e.target.value))}
														/>
													</FormControl>
													<FormMessage />
												</FormItem>
																	...
																	<CommandGroup>
																		{rentalTypes.map((type) => (
																			<CommandItem
																				key={type}
																				value={type}
																				onSelect={(currentValue) => form.setValue("rentalType", currentValue)}
																			>
																				<Check
																					className={cn("mr-2 h-4 w-4", form.watch("rentalType") === type ? "opacity-100" : "opacity-0")} />
																				{type}
																			</CommandItem>
																		))}
																	</CommandGroup>
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem>
													<FormLabel>Loại người thuê</FormLabel>
													<Popover>
														<PopoverTrigger asChild>
															<FormControl>
																<Button variant="outline" role="combobox"
																	...
																	<CommandGroup>
																		{tenantTypes.map((type) => (
																			<CommandItem
																				key={type}
																				value={type}
																				onSelect={(currentValue) => form.setValue("tenantType", currentValue)}
																			>
																				<Check
																					className={cn("mr-2 h-4 w-4", form.watch("tenantType") === type ? "opacity-100" : "opacity-0")} />
																				{type}
																			</CommandItem>
																		))}
																	</CommandGroup>
								...
								<CardFooter className="flex justify-end pt-6">
									<Button type="submit" className="w-full sm:w-auto">
										{isFetching ? "Đang cập nhật..." : "Lưu thay đổi"}
									</Button>
								</CardFooter>
							</form>
						</Form>
