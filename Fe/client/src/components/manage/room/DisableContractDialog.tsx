...
function DisableContractDialog({ contractId }: { contractId: string }) {
	...
	async function handleDisable() {
		...
		if (res?.code === 0) {
			toast({ description: "Vô hiệu hóa hợp đồng thành công" });
		} else {
			toast({ description: "Vô hiệu hóa hợp đồng thất bại" });
		}
	}
	...
	return (
		<AlertDialog>
			...
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>Bạn chắc chắn muốn hủy hợp đồng?</AlertDialogTitle>
					<AlertDialogDescription>
						Khi hủy thì không thể khôi phục lại hợp đồng như ban đầu
					</AlertDialogDescription>
				</AlertDialogHeader>
				<AlertDialogFooter>
