...
public class UserVerificationRequestController {
    ...
    @PostMapping
    public ApiResponse<?> updateAvatar(@RequestParam("image") MultipartFile file, @RequestParam String cardId) {
    ...
    public ApiResponse<?> verifyUser(
            @RequestBody @Valid UserVerificationUpdateRequest request, @RequestHeader("Authorization") String token) {
        return ApiResponse.builder()
                .result(userVerificationRequestService.verifyUser(request, token))
                .build();
    }
    ...
    public ApiResponse<?> getAllUserVerificationRequest(
        ...
        return ApiResponse.builder()
                .result(userVerificationRequestService.getALl(
                        pageNum, pageSize, sortBy, order, search, userId, isChecked, isSuccessful))
                .build();
    }

    @GetMapping("/{id}")
    public ApiResponse<?> getUserVerificationRequestById(@PathVariable String id) {
        return ApiResponse.builder()
                .result(userVerificationRequestService.getById(id))
                .build();
    }
    ...
    public ApiResponse<?> getUserVerificationRequestByUserId() {
        return ApiResponse.builder()
                .result(userVerificationRequestService.get())
                .build();
    }

    @GetMapping("/has-permission")
    public ApiResponse<Boolean> hasPermissionToViewIdCard(
            @RequestParam String imgUrl, @RequestHeader("Authorization") String token) {
