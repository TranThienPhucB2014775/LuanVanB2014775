...
import { Info, Users, Calendar, Home, DollarSign } from 'lucide-react';
...
function CurrentRoomsPage() {
	...
	const [data, setData] = useState<Array<TenantRoomResponse> | undefined>(undefined);
	...
	const [params, setParams] = useState<string>("?isAvailable=true");
	...
	useEffect(() => {
		async function fetchData() {
			...
			setPage(prev => ({ ...prev, totalPage: res?.payload?.result.totalPage || 0 }));
		}
	...
	const getRentStatusColor = (status: boolean) => {
		return status ? "text-green-500" : "text-yellow-500";
	};

	return (
		<div className="container mx-auto px-4 py-8">
			<h1 className="text-3xl font-bold mb-6 text-primary">Phòng đang thuê</h1>
			{isFetching ? (
				<LoadingSpinner />
							...
							<CardHeader className="pb-2">
								<div className="flex justify-between items-start">
									<div>
										...
										<Badge variant={room.rentStatus === "RENTED" ? "default" : "secondary"}>
											{room.rentStatus === "RENTED" ? "Đang thuê" : "Đã trả phòng"}
										</Badge>
									</div>
