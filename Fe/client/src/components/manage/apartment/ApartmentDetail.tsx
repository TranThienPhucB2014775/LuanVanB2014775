		...
		<div className="space-y-6 py-4">
			<Card>
				...
				<CardContent>
                    ...
                    <XCircle className="w-4 h-4" /> Ngưng hoạt động
							...
							<p className="flex items-center gap-2">
								<strong>Số người ở hiện tại:</strong>
								<span className="flex items-center gap-1">
                  <Users className="w-4 h-4" />
									{res.payload?.result.currentOccupancy}
                </span>
							</p>
						</div>
						<div>
							...
							<p>{renderTextWithLineBreaks(res.payload?.result.rule)}</p>
						</div>
					</div>
				</CardContent>
			</Card>
			...
			{res.payload?.result.description && (
				<Card>
					...
					<CardContent>
						<p>{renderTextWithLineBreaks(res.payload?.result.description)}</p>
					</CardContent>
				</Card>
			)}
			...
			{invoiceUnPaid.payload?.result && < InvoiceUnPaidCard invoiceUnPaid={invoiceUnPaid.payload?.result} />}
			<ListAdditionalCost
