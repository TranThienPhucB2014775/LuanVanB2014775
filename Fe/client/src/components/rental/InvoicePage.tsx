		...
		<PageAnimate>
			<div className="container mx-auto p-4">
				...
				{isFetching
					...
					: data ? (
							<>
								<Card className={`mb-6 ${!data.isPaid ? "border-red-500" : ""}`}>
									<CardHeader>
										<CardTitle>Tóm Tắt Chi Phí cho {months[parseInt(month) - 1]} {year}</CardTitle>
									</CardHeader>
									<CardContent>
										<p className={`text-2xl font-bold ${!data.isPaid ? "text-red-500" : ""}`}>
											{data.cost.toLocaleString()} VND
										</p>
										...
										<p className={`text-sm ${!data.isPaid ? "text-red-500 font-bold" : "text-muted-foreground"}`}>
											Trạng thái: {data.isPaid ? "Đã thanh toán" : "Chưa thanh toán"}
										</p>
									</CardContent>
								</Card>

								<Card className={`${!data.isPaid ? "border-red-500" : ""}`}>
									<CardHeader>
										...
										<Table>
											<TableHeader>
												<TableRow>
													...
													<TableHead>Sử Dụng/Chỉ số</TableHead>
											...
											<TableBody>
												{data.monthlyUsageResponses.map((item, index) => (
													<TableRow key={index} className={!data.isPaid ? "text-red-500" : ""}>
