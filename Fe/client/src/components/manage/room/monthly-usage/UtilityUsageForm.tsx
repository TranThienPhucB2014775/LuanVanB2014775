...
function UtilityUsageForm({ roomId, handleAddInvoice }: { roomId: string, handleAddInvoice: Function }) {
	...
	const { error: errorCompleteInvoice, isFetching: isFetchingCompleteInvoice, fetch: completeInvoice } =
		useFetch<ApiResponse<result<invoiceType>>>(
			(data: { month: number; year: number; roomId: string; sessionToken: string }) =>
	...
	async function completeInvoiceData() {
		...
		if (res?.code === 0) {
			...
			handleAddInvoice(res.payload?.result);
			setIsCompleted(true);
			toast({
